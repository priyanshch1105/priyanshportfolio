...
const ContactScene = dynamic(() => import("@/components/three/ContactScene").then((m) => m.ContactScene), {
  ssr: false,
  loading: () => null,
})
...
const SOCIALS = [
  { label: "GitHub", href: site.socials.github.href, icon: Github, handle: site.socials.github.handle },
  { label: "LinkedIn", href: site.socials.linkedin.href, icon: Linkedin, handle: site.socials.linkedin.handle },
  { label: "Email", href: `mailto:${site.email}`, icon: Mail, handle: site.email },
  { label: "Instagram", href: site.socials.instagram.href, icon: Instagram, handle: site.socials.instagram.handle },
]
...
export function Contact() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (isReducedMotion()) {
        gsap.set(".contact-letter, .contact-fade", { autoAlpha: 1, yPercent: 0, y: 0 })
        return
      }
  ...
  return (
    <section ref={rootRef} id="contact" className="relative flex min-h-[100svh] flex-col overflow-hidden pt-32 md:pt-40">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col px-5 md:px-10">
        <div className="mb-10 flex items-center gap-4 md:mb-16">
          <span className="font-mono text-[11px] uppercase tracking-[0.3em] text-accent">08</span>
          <span className="h-px flex-1 bg-line" />
          <span className="font-mono text-[11px] uppercase tracking-[0.3em] text-dim">Contact</span>
        </div>
          ...
          <div className="mt-12 flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
            <div className="contact-fade max-w-sm">
              <p className="font-display text-lg leading-relaxed text-mute md:text-xl">
                Have a product, an experiment or a moonshot in mind? My inbox is always open.
              </p>
            ...
            <div className="contact-fade flex flex-col items-start gap-5 md:items-end">
              <MagneticCTA href={`mailto:${site.email}`} variant="solid">
                Let&apos;s Connect
              </MagneticCTA>
              ...
              <div className="mt-2 flex flex-wrap gap-3">
                {SOCIALS.map((social) => (
                  <Magnetic key={social.label} strength={0.3}>
                    ...
                    >
                      <social.icon size={15} className="text-mute transition-colors group-hover:text-accent" />
      ...
      <div className="contact-fade relative z-10 mx-auto mt-20 w-full max-w-7xl px-5 pb-8 md:px-10">
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-line pt-6">
          <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
            {site.name} · Frontend · AI/ML · Mobile
          </span>
          <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
